...
def tree_shape_draw_function():
    ...
    while i < len(tree_positions_list):
        ...
        GL.glBegin(GL.GL_QUADS) #tree body create
        ...
        GL.glBegin(GL.GL_TRIANGLES) #tree leaves create
...
def direction_change_condition(arrow_key_bind):
    ...
    if arrow_key_bind == GLUT.GLUT_KEY_LEFT:
        raindrop_direction_change_counter=max(raindrop_direction_change_counter - 0.5, -2.5) #-2.5 max
        print("Raindrop direction change to left")
    elif arrow_key_bind == GLUT.GLUT_KEY_RIGHT:
        raindrop_direction_change_counter=min(raindrop_direction_change_counter + 0.5, 2.5) #2.5 max
        print("Raindrop direction change to right ")
...
def update_raindrop_condition(x,y):
    ...
    if y < 0:
        ...
        y = random.uniform(0, 900)   #raindrop screen ar baire gele ekhane ashbe
...
def raindrop_animation_display():
    ...
    updated_raindrops_list = []  #new raindrop list
...
def raindrop_display_loop(updated_raindrops_list):
    for x, y in rain_drop_speed_list:
        ...
        updated_raindrops_list.append((new_x_axis_coordinate, new_y_axis_coordinate)) #notun list a append
...
#     def create_new_point(self, x, y): #function when right mouse button is clicked
#         direction_x = random.choice([-1, 1]) * random.uniform(0.3, 0.7) #random direction for x-axis
#         direction_y = random.choice([-1, 1]) * random.uniform(0.3, 0.7) #random direction for y-axis
#         color = (random.random(), random.random(), random.random())  #point color
